...
describe('unified tools', () => {
  ...
  it('generates action-specific schemas for asset and prefab tools', () => {
    ...
    const createAsset = assetManage?.inputSchema.oneOf?.find(schema => schema.properties?.action?.enum?.[0] === 'create')
    ...
    expect(createAsset?.properties?.content).toMatchObject({ oneOf: expect.arrayContaining([{ type: 'string' }, { type: 'object' }, { type: 'array' }]) })
  ...
  it('generates action-specific schemas for scene-control tools', () => {
    ...
    expect(getTree?.properties).toHaveProperty('maxNodes')
  ...
  it('generates action-specific schemas for diagnostics and utility tools', async () => {
    ...
    const debugGet = debugLogs?.inputSchema.oneOf?.find(schema => schema.properties?.action?.enum?.[0] === 'get')
    ...
    expect(debugGet).toMatchObject({ required: ['action'], additionalProperties: false })
    expect(debugGet?.properties).toHaveProperty('maxChars')
    expect(debugGet?.properties?.lines).toMatchObject({ minimum: 1, maximum: 10000 })
    expect(debugSearch).toMatchObject({ required: ['action', 'pattern'], additionalProperties: false })
    expect(debugSearch?.properties).toHaveProperty('maxChars')
    expect(debugSearch?.properties).toHaveProperty('startLine')
